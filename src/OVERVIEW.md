...
Run code agents directly from your Azure DevOps pipelines to generate code based on work items or custom prompts and (optionally) create pull requests with the changes, or run it in prResponse mode and it will comment on the pull request with the generated code.
...
- Ability to respond to pull request comments in prResponse mode, implement the changes suggested in the comments and reply back with the updated code.

![Autocoder Pull Request Example](screenshots/pull-request-claude.png)
![Autocoder PR Response Example](screenshots/pull-request-response.png)
...
Here is the same task, but configured to respond to pull request comments in prResponse mode:
```yaml
trigger:
- none
...
variables:
  ...
  - name: pullRequestId
    ${{ if eq(parameters.PullRequestId, 0) }}:
      value: ''
    ${{ else }}:
...
Here is an example of how to use the Autocoder task with Claude Code in a YAML pipeline:
...
|-------------|---------------------------------------------|----------|---------|
| Operation Mode | The mode in which to run the task. 'codeGen' for generating code based on work items or prompts, 'prRespond' for responding to pull request comments. | Yes      | codeGen |
...
| Pull Request ID (prRespond mode) | The ID of the pull request to respond to when running in prRespond mode. | No (required in prRespond mode) | 42 |
